...
def do_preprocess(tokenizer, dataset_split):
    ...
    for json_file in tqdm(json_files):
        ...
        out_json_obj["words"] = []
        out_json_obj["parse"] = {}
        out_json_obj["parse"]["class"] = { "question": [], "answer": []}
        ...
        for form_idx, form in enumerate(in_json_obj):
            ...
            is_valid_class = False if form_label == "" else True
            if is_valid_class:
        ...
        out_json_obj["meta"] = {}
        image_file = (
            os.path.splitext(json_file.replace("/annotations/", "/images/"))[0] + ".jpg"
        )
        if dataset_split == "train":
